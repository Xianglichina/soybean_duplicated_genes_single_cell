...
early_nodule_acrs <- extract_tissue_acrs(acr_matching_rna_normalized, "Early_nodule")
root_acrs <- extract_tissue_acrs(acr_matching_rna_normalized, "Root")
hypocotyl_acrs <- extract_tissue_acrs(acr_matching_rna_normalized, "Hypocotyl")
```
...
long_gene_pair_early_nodule <- connect_acrs_genepairs(early_nodule_acrs, "early_nodule")
long_gene_pair_root <- connect_acrs_genepairs(root_acrs, "root")
long_gene_pair_hypocotyl <- connect_acrs_genepairs(hypocotyl_acrs, "hypocotyl")

# Combine all tissues into one dataframe
all_tissues_acrs <- bind_rows(
  long_gene_pair_heart,
  long_gene_pair_early_nodule,
  long_gene_pair_root,
  long_gene_pair_hypocotyl
)
...
cat("Number of gene pairs analyzed:", nrow(variability_results), "\n")
...
variability_with_stability <- variability_results %>%
  ...
  filter(!is.na(Tissue_Stability_Score))  # Keep only gene pairs with stability data
...
summary_by_tissue_stability <- variability_with_stability %>%
...
ggplot(variability_with_stability, aes(x = factor(Tissue_Stability_Score), y = jaccard_variability)) +
...
cat("\nSummary by Tissue Stability:\n")
print(summary_by_tissue_stability)
```
...
write.csv(variability_results, "09042025_aacr_variability_results_simplified_tissue.csv", row.names = FALSE)
write.csv(variability_with_stability, "09042025_aacr_variability_with_stability_tissue.csv", row.names = FALSE)
write.csv(summary_by_tissue_stability, "09042025_asummary_by_tissue_stability.csv", row.names = FALSE)

cat("\nResults saved to CSV files:\n")
cat("- acr_variability_with_stability.csv: Combined variability and stability data\n")
cat("- summary_by_tissue_stability.csv: Summary statistics by tissue stability groups\n")

cat("\n=== TISSUE-VARIABILITY ANALYSIS COMPLETE ===\n")
cat("This analysis explores the relationship between ACR variability and gene expression stability across tissues.\n")
cat("Use the group comparisons to understand how regulatory variability relates to expression stability.\n")
```
